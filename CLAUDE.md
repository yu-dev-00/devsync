...
- **Exec streams output.** The agent `.spawn()`s the command and drains stdout *and* stderr on two reader threads — draining only one deadlocks when the child fills the other. The threads forward raw bytes to the main thread, which owns the writer; frames written from two threads would interleave mid-frame. Child stdin is `Stdio::null()`: `.output()` did that implicitly, but an inherited handle would let the child consume the agent's own protocol stream. Chunk boundaries split multi-byte sequences, so `OutputDecoder` carries the incomplete tail into the next chunk — decoding each chunk independently corrupts characters that straddle the boundary.
- **Command output has no single encoding**, so `OutputDecoder` decides one per line: valid UTF-8 is UTF-8, anything else is the console code page. Both writers appear on one pipe — PowerShell and the classic console tools emit the console output code page (CP932 on a Japanese install), while Rust/Go tools (`uv`, `cargo`) emit UTF-8 and ignore that setting — so *either* fixed choice garbles half the output, and both regressions have already happened. `console_encoding()` supplies the fallback by asking Windows (`GetConsoleOutputCP`, falling back to `GetOEMCP` because the agent has no console under sshd). What makes the per-line verdict safe is that every segment `decode` sees is self-contained: lines split on `\n`/`\r`, which are never part of a character in either encoding, and the unterminated tail holds back bytes a later chunk may complete. Do not reintroduce a decoder whose state spans a verdict. Note this is all separate from *script file* encoding: PowerShell 5.1 reads a BOM-less `.ps1` as ANSI, which garbles literals before they are ever printed.
- **Exec checks `$?` before forwarding `$LASTEXITCODE`.** Native failures keep their exact code (for example, cargo's 101). A failing cmdlet or command lookup with a null/zero native code returns 1 instead of silently succeeding. The wrapper starts on a new line so a trailing command comment cannot swallow it.
- **Windows path identity is case-insensitive.** Diff and excludes share ordinal comparison in `path_safety::PathKey`; the original spelling is retained for display and I/O. Case-only name differences never schedule deletion of the same file.
- **File/directory transitions require ordered sync batches.** `sync::apply_plan` sends collision-related, explicitly planned deletions first and waits for their ack before uploading; unrelated deletions remain last. The agent removes only empty, non-excluded directory trees to make room for a file. It never recursively deletes residual files or follows links during that cleanup. Existing files blocking a transition require `--delete`.
- **Ctrl+C needs no protocol support**, however much it looks like it should. The agent does not read stdin while a command runs and there is no `Interrupt` message, so the obvious conclusion is that interrupting a long build orphans it remotely. Measured against a real remote instead (2026-08-07): killing the local `devsync.exe` together with its `ssh.exe` — which is what Ctrl+C causes, since a console signals every process attached to it — takes the remote agent *and* its whole process tree (`powershell.exe` → `ping.exe`) down within seconds. Windows OpenSSH's sshd owns that teardown. So an `Interrupt` frame, a job object around the child, and the protocol bump they would cost buy nothing; do not add them without re-measuring first. An interrupted `sync` self-heals for a separate reason: the diff is content-hash based, so whatever did not land is simply re-uploaded next time.
